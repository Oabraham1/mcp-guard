...
fn summarize_threats(threats: &[scanner::Threat]) -> String {
    ...
    let by_severity: std::collections::HashMap<Severity, usize> =
        threats.iter().fold(std::collections::HashMap::new(), |mut acc, t| {
        ...
        "runs": [{
            "tool": {
                "driver": {
                    ...
                    "informationUri": "https://github.com/oabraham1/mcp-guard"
                }
            },
...
async fn cmd_watch(clients: Option<String>) -> Result<()> {
    ...
    let config_paths: HashSet<std::path::PathBuf> = servers
        .iter()
    ...
    for path in &config_paths {
        watcher
            ...
            .map_err(|e| error::Error::Other(format!("Failed to watch {}: {}", path.display(), e)))?;
    }
    ...
    for server in servers {
        match scanner.scan(server).await {
            Ok(result) => {
                let status = if result.threats.is_empty() {
                    "✓".green()
                } else if result.threats.iter().any(|t| t.severity == Severity::Critical) {
                    "✗".red()
                } else {
                    "⚠".yellow()
                };
            ...
            Err(e) => {
                eprintln!("{} {}: {}", "✗".red(), server.name.bold(), e.to_string().red());
            }
        }
    }
}
...
async fn cmd_proxy(
    ...
    eprintln!("{}", format!("Proxying server: {} {}", command, args.join(" ")).cyan());
...
async fn cmd_serve(port: u16, bind: String, headless: bool) -> Result<()> {
    ...
    println!("{}", format!("Starting mcp-guard server at {}", url).cyan());
...
fn cmd_init(force: bool) -> Result<()> {
    let home = dirs::home_dir().ok_or_else(|| error::Error::Other("No home directory".to_string()))?;
    let config_dir = home.join(".mcp-guard");
    ...
    let default_config = r#"# mcp-guard configuration
...
fn cmd_completions(shell: cli::Shell) {
    ...
    generate(shell.to_clap_shell(), &mut cmd, "mcp-guard", &mut io::stdout());
}
