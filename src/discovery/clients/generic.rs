...
pub fn parse_mcp_servers(
    ...
    let config: McpConfig =
        serde_json::from_str(content).map_err(|e| Error::config_parse(config_path, e.to_string()))?;
...
mod tests {
    ...
    fn parse_config_with_env() {
        ...
        let servers = parse_mcp_servers(json, &PathBuf::from("/test/config.json"), "test").unwrap();
        assert_eq!(servers[0].env.get("GITHUB_TOKEN"), Some(&"secret".to_string()));
    }
    ...
    fn parse_remote_server() {
        ...
        let servers = parse_mcp_servers(json, &PathBuf::from("/test/config.json"), "test").unwrap();
        assert!(matches!(&servers[0].transport, TransportType::Sse { url } if url == "http://localhost:8080"));
    }
